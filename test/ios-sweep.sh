...
# points (402 x 874); the tab bar centres are y=812, x=43,122,201,280,359 (five tabs).
set -euo pipefail
U="${1:-5982E2BF-C16C-4243-B049-26D1FA73FC07}"
OUT="${2:-design/screenshots/sweep-$(date +%Y-%m-%d)}"
mkdir -p "$OUT"
TABS="43:home 122:focus 201:learn 280:friends 359:kin"
...
sweep(){ for t in $TABS; do tap "${t%%:*}" 812; shot "$1-${t##*:}"; done; }
...
tap 36 812; sleep 1; tap 326 81;  shot light-shop        # coin chip -> Shop
idb ui swipe 200 120 200 800 --udid "$U"; sleep 1
...
tap 36 812; sleep 1; tap 326 81;  shot axxl-shop                # the sheets grow too
idb ui swipe 200 120 200 800 --udid "$U"; sleep 1
tap 369 429; shot axxl-yourday
tap 334 110; sleep 1                                            # Done (the sheet is taller at AX-XL)
tap 234 812; sleep 1; tap 201 275; sleep 1.5
