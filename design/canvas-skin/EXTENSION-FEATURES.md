...
| Handing in | Duolingo's XP moment | The page notices a submission (classic forms under `#submit_assignment`, or `button[data-testid="submit-button"]`), the worker syncs once (a minute's rate limit per school), the ring moves, the buddy cheers and **+30** floats from him. R24 | `content.js` `watchHandIn`/`syncIfJustHandedIn`, `background.js` `syncAfterSubmit`, `test/fake-canvas.js` |
| Running session | Oura + the rail | The rail's top row becomes the session under **Now** (task, clock ticking in place, Open, Stop); the queue skips it; a second Focus press keeps the running one. Clocks tick in place, nothing is rebuilt each second. R25 | `content.js` `renderWeek`, `tickClocks`, `background.js` `focusStart` |
| Card rows | Todoist hover actions | A pending DUE row is a link; on hover its date steps aside for Start. R26 | `content.js` `decorateCards`, `skin.css` `a.pk-due-row` |
