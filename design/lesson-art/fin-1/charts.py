...
class Canvas:
    ...
    def save(self, name):
        self.im.resize((W, H), Image.LANCZOS).save(os.path.join(OUT, name), "PNG")
        print("wrote", name)
...
def chart_thirty():
    ...
    for t in (10, 20, 30):
        ...
        c.text(money(v), x - (30 if t == 30 else 0), y - 44, 32, anchor="mm" if t < 30 else "rm")
...
def chart_simple_vs_compound():
    ...
    c.text("$3,400  simple", x1 - 10, ys + 44, 30, SKY, anchor="rm")
