...
const RULES = [
  ...
  { key: 'due-column', kind: 'fixed', label: 'Due dates and points in one column, in readable type; no dash where nothing is scored yet', hook: 'moduleDue' },
  { key: 'module-band', kind: 'fixed', label: 'The bar above a list, folded onto the title line: Collapse All, Show by, the search', hook: 'moduleBar' },
  { key: 'buttons', kind: 'fixed', label: "Canvas's plain buttons on the paper; submit buttons stay Canvas's", hook: 'plainButton' },
  { key: 'feedback-rows', kind: 'fixed', label: 'Recent Feedback as rows, the mark on its line', hook: 'recentFeedback' },
  ...
  { key: 'grades-page', kind: 'taken', label: "Canvas's grades table, folded under your own grade row, with its side column", detect: 'gradesPage' },
  { key: 'list-view', kind: 'taken', label: "Canvas's List View, folded under the Planner", detect: 'listView' },
...
function skinClasses({ on, dark, look, dense = false, hidePast = false, gradeHover = false, putBack = {}, detect = {} }) {
  ...
  // The icon set follows the look (themes.js `icons`), and on the dark stocks
  // and the image looks a set with a solid family wears it (pk-icons-soft).
  classes.push(`pk-icons-${look?.icons ?? 'plump'}`);
